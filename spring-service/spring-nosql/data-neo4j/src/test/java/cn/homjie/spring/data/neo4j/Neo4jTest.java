...
import cn.homjie.spring.data.neo4j.dao.OrderRepository;
import cn.homjie.spring.data.neo4j.domain.Item;
...
//		assertEquals(2, chucksOrders2.get(0).getItems().size());
	...
	private Order createAnOrder() {
		...
		order.setItems(new HashSet<>(Arrays.asList(item1, item2)));
		return order;
	}
